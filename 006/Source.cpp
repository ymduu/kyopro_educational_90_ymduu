...
普通の行列積(N, +, *) の場合は単に l, r を渡すだけでいけることに注意すること
...
vector<Vector2D> ConvexHull(vector<Vector2D> ps) {
	...
	int k = 0;					// 凸包の頂点数
	vector<Vector2D> qs(n * 2);	// 構築中の凸包

								// 下側
	for (int i = 0; i < n; i++) {
		// 外積が負 -> 引数側ベクトルが時計回りの方向にある -> 凹んでいるので直前の頂点を除く
...
void solve(ostringstream& aout, long long N, long long M, std::string S, std::string T);
void solve_TLE(ostringstream& aout, long long N, long long M, std::string S, std::string T);

class StressTest {
private:
	mt19937 m_RandEngine;
	bool judge_case(long long N, long long M, std::string S, std::string T) {
		ostringstream fast, tle;
		solve(fast, N, M, S, T);
		solve_TLE(tle, N, M, S, T);
		if (fast.str() == tle.str()) {
			return true;
		}
		else {
...
signed main() {
	...
	rep(i, K) {
		for (char c = 'a'; c <= 'z'; c++) {
			while (ma[c][nowIdx[c]] != LL_HALFMAX) {
				if (maxIdx > ma[c][nowIdx[c]]) {
					nowIdx[c]++;
				}
				else {
					break;
			...
			if (ma[c][nowIdx[c]] == LL_HALFMAX) {
				continue;
			}

			int idx = ma[c][nowIdx[c]];
			if (K - i - 1 > len - idx - 1) {
				continue;
			}
