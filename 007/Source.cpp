...
void solve(ostringstream& aout, long long S);
...
void solve(ostringstream& aout, long long S){
	...
	REPS(len, 3000) {
		...
		// target 個の まる と len - 1 個のしきり
		mint add = binom(target + len - 1, target);
		ans += add;
...
signed main() {
	...
	for (auto q : B) {
		auto it = lower_bound(ALLOF(A), q);
		int c1 = *it;
		it--;
		int c2 = *it;

		int ans = min(abs(q - c1), abs(q - c2));
		cout << ans << "\n";
	}
